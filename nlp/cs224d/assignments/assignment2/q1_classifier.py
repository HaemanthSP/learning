...
class SoftmaxModel(Model):
  ...
  def load_data(self):
    ...
    # Experimenting
    #self.input_labels = np.array([np.random.randint(5) for i in range(self.config.n_samples)], dtype=np.int32)
  ...
  def add_training_op(self, loss):
    ...
    grad_var_pair = optimizer.compute_gradients(loss, [self.weights, self.bias])
  ...
  def add_model(self, input_data):
    ...
    """
    ### YOUR CODE HERE
    self.weights = tf.Variable(tf.zeros((self.config.n_features, self.config.n_classes), dtype=tf.float32))
    self.bias = tf.Variable(tf.zeros((1, self.config.n_classes), dtype=tf.float32))
    score = tf.matmul(input_data, self.weights) + self.bias
    out = softmax(score)
    ### END YOUR CODE
    return out
  ...
  def fit(self, sess, input_data, input_labels):
    ...
    print sess.run([self.weights, self.bias])
    return losses
